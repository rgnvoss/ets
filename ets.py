...
LNAME = "Email Testing System"
SNAME = "ETS"
VERSION = "1.0.6"
BUILD = "220928.02"
SUFFIX = "Stable"
PNAME = LNAME + " (" + SNAME + ") v" + VERSION + "-" + BUILD + " " + SUFFIX
...
def mime_headers(mime_multipart, mime_msg_id, mime_xmailer, mime_timestamp, mime_subject, mime_from_header, mime_to_header, mime_importance, mime_priority, mime_return, mime_reply):
	...
	try:
		...
		mime_msg['Return-PATH'] = mime_return
		mime_msg['Reply-To'] = mime_reply
...
def send_email(send_target, send_port, send_sender, send_recipient, send_body, send_tls):
	...
	try:
		...
		server.sendmail(send_sender, send_recipient.split(","), send_body)
		server.quit()
		sys.stderr.flush()
		t.flush()
		t.seek(0)
...
Envelope Options:
  -t, --to, --recipient                  Envelope recipient(s)(required to
                                         send). Multiple recipients should
                                         be separated with commas and no
                                         spaces.
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(argv,"vh:t:p:r:f:e:x:l:",["version","server=","target=", \
			"port=","to=","recipient=","from=","sender=","loop=","ehlo=","helo=","to-header=", \
			"from-header=","subject=","ssl","tls","spam","adult","virus","av","url","zip", \
			"eml","write","no-send","eml-name=","no-text","no-html","xm=","x-mailer=", \
			"text-encode=","text-charset=","html-encode=","html-charset=","encode=","charset=", \
			"body-text=","body-html=","ssn","mix","mixed","high","low","text-body=", \
			"html-body=","attach=","log","log-level=","log-file="])
	...
	if eml_send:
		if target == "smtp.example.com" or recipient == "recipient@example.com":
			logging.info("Unable to generate email: Target server and recipient are required to send.")
			sys.exit("Unable to send email: Target server and recipient are required to send.\r\nExiting.")
	...
	msg = mime_headers(multi_type, msg_id, xmailer, timestamp, subject, from_header, to_header, msg_importance, msg_priority, return_header, reply_header)
	...
	if looping:
		...
		logging.debug("Looping " + loop_count + " times")

		for x in range(1, int(loop_count)+1):

			logging.debug("Sending email #" + str(x))
			if eml_send:
				send_email(target, port, sender, recipient, body, tls)
				logging.debug("Email #" + str(x) + " sent")

			if eml_file:
				write_eml_file(str(x) + "-" + eml_name, body)
				logging.debug("Email #" + str(x) + " written to " + eml_name)

	else:

		if eml_send:
			if target == "smtp.example.com" or recipient == "recipient@example.com":
				sys.exit("Target server and recipient are required.\r\nExiting.")
			logging.debug("Sending email")
			send_email(target, port, sender, recipient, body, tls)
			logging.debug("Email sent")

		if eml_file:
			logging.debug("Writing email to file")
			write_eml_file(eml_name, body)
			logging.info("Email written to " + eml_name)
